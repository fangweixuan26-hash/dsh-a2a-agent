 ...
 *     name: dsh-a2a-agent
 *     config:
 *       enableStreaming: true    # opt-in SSE message/stream
 *
 * Endpoints:
 ...
 *
 * JSON-RPC methods: message/send, message/stream (SSE, when enabled),
...
export default function apply(ctx, config) {
  ...
  // 可选择启用：在 cordis.yml 的 config.enableStreaming 设为 true 开启 SSE 流式。
  const enableStreaming = !!(config && config.enableStreaming)
